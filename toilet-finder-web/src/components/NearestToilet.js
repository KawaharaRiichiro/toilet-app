    ...
    async function fetchToilets() {
      ...
      try {
        ...
        const uniqueStationToilets = stationToilets.filter(st => {
          return !publicToilets.some(pt => {
            const dist = calculateDistance(st.latitude, st.longitude, pt.latitude, pt.longitude);
            return dist < 30;
          });
        });
        
        const withDistance = [...publicToilets, ...uniqueStationToilets].map(t => ({
          ...t,
          ...
          {toilets.map((toilet) => (
            <Marker
              ...
              icon={{
                  ...
                  ? "https://maps.google.com/mapfiles/ms/icons/purple-dot.png" // 紫
                  : "https://maps.google.com/mapfiles/ms/icons/red-dot.png" // 赤
              }}
            />
          ))}
       </GoogleMap>
